...
/*---------------------------------------------------------*/
    ...
    imgWeather->setPixmap(QPixmap(":/images/sun.png"));
...
/*-----------------------------------------------------------*/
    ...
    sectionLayouts[BAROMETER]->addWidget(sensorsValue[BAROMETRIC_PRESSURE_SENSOR], 1, 0,1,
                                         2, Qt::AlignVCenter);
    /*-----------------------------------------------------------*/
    graphic = new QtCharts::QChart;
    lineGraphic = new QtCharts::QLineSeries;
    QColor colorLine;
    graphic->addSeries(lineGraphic);

    QBrush  brushBackground(QColor().fromRgb(3, 3, 6));
    brushBackground.setStyle(Qt::SolidPattern);

    graphic->setBackgroundVisible(false);
    graphic->setPlotAreaBackgroundBrush(brushBackground);

    QBrush  brushText(QColor().fromRgb(255, 255, 255));
    brushBackground.setStyle(Qt::SolidPattern);

    graphic->setTitle("Temperature graph for 1 hour");
    graphic->setTitleBrush(brushText);


    QFont fontHeader;
    fontHeader.setPixelSize(20);
    graphic->setTitleFont(fontHeader);
    ...
    axisX->setTitleText("x, seconds");
    axisX->setLabelFormat("%.2f");
    axisX->setLabelsBrush(brushText);
    axisX->setTitleBrush(brushText);
    axisX->setTitleFont(fontText);
    axisX->setLabelsFont(fontText);
    graphic->addAxis(axisX, Qt::AlignBottom);

    lineGraphic->attachAxis(axisX);

    auto *axisY = new QtCharts::QValueAxis;
    axisY->setRange(-100, 100);
    axisY->setTickCount(10);
    axisY->setTitleText("y, °C");
    axisY->setLabelFormat("%.2f");
    axisY->setLabelsBrush(brushText);
    axisY->setTitleBrush(brushText);
    axisY->setTitleFont(fontText);
    axisY->setLabelsFont(fontText);
    graphic->addAxis(axisY, Qt::AlignLeft);

    lineGraphic->attachAxis(axisY);

    auto *graphicView = new QtCharts::QChartView(graphic);

    labelSections[GRAPH] = new QLabel("GRAPH");
    sectionLayouts[GRAPH]->addWidget(labelSections[GRAPH], 0, 0);
    sectionLayouts[GRAPH]->addWidget(graphicView);
