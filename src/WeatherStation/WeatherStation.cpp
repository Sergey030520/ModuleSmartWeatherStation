...
void WeatherStation::setValSensorsOnWin(MessageSensors &inMessage) {
    ...
    int currentSec = QTime::currentTime().minute()*60 + QTime::currentTime().second();
    if(currentSec != preSec) {
        updateDataGraphics((float)currentSec, inMessage[TEMPERATURE_SENSOR]);
        preSec = currentSec;
    }
...
void WeatherStation::updateDataGraphics(float x, float y) {
    lineGraphic->append(x, y);
    if(lineGraphic->count() >= 100){
        lineGraphic->remove(0);
    }
    graphic->update();
}
